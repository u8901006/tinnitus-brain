#!/usr/bin/env python3
"""
Generate tinnitus daily report HTML using Zhipu AI (GLM-5-Turbo).
Reads papers JSON, analyzes with AI, generates styled HTML matching Psychiatry-brain design.
"""
...
SYSTEM_PROMPT = (
    "你是耳鳴／聽覺神經科學領域的資深研究員與科學傳播者。你的任務是：\n"
    "1. 從提供的醫學文獻中，篩選出最具臨床意義與研究價值的耳鳴相關論文\n"
...
TINNITUS_TAGS = [
    "耳鳴機制", "聽覺神經科學", "聽力損失", "過敏性聽覺", "恐音症",
    "認知行為治療", "正念", "聲音治療", "助聽器", "耳蝸植入",
    "神經調節", "rTMS", "tDCS", "雙模態刺激",
    "體感耳鳴", "顳顎關節", "頸椎", "物理治療",
    "憂鬱症", "焦慮症", "失眠", "創傷後壓力症",
    "營養", "飲食", "咖啡因", "鎂", "鋅", "維生素",
    "運動", "身體活動", "復健",
...
def robust_json_parse(text: str) -> dict:
    ...
    print("[ERROR] All JSON repair attempts failed", file=sys.stderr)
    return None
...
def analyze_papers(api_key: str, papers_data: dict) -> dict:
    ...
    tags_list = "、".join(TINNITUS_TAGS)

    prompt = f"""以下是 {date_str} 從 PubMed 抓取的最新耳鳴／聽覺研究文獻（共 {paper_count} 篇）。
...
每篇 paper 的 tags 請從以下選擇：{tags_list}
        ...
        for attempt in range(3):
            try:
                ...
                if result is not None:
                    print(
                        f"[INFO] Analysis complete with {model}: {len(result.get('top_picks', []))} top picks, {len(result.get('all_papers', []))} total",
                        file=sys.stderr,
                    )
                    return result
                else:
                    print(
                        f"[WARN] JSON parse failed for {model} attempt {attempt + 1}, retrying...",
                        file=sys.stderr,
                    )
                    if attempt < 2:
                        time.sleep(5)
...
def generate_html(analysis: dict) -> str:
    ...
    for pick in top_picks:
        ...
        <div class="news-card featured">
          ...
          {pico_html}
          <p class="utility-reason">💡 {pick.get("utility_reason", "")}</p>
...
<title>Tinnitus Brain &middot; 耳鳴研究文獻日報 &middot; {date_display}</title>
<meta name="description" content="{date_display} 耳鳴研究文獻日報，由 AI 自動彙整 PubMed 最新耳鳴、聽覺神經科學論文"/>
<style>
  ...
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .links-banner {{ margin-top: 32px; animation: fadeUp 0.5s ease 0.3s both; }}
  .links-banner .link-row {{ display: flex; gap: 12px; margin-bottom: 10px; }}
  .links-banner a {{ flex: 1; display: flex; align-items: center; gap: 12px; padding: 16px 20px; background: var(--card-bg); border: 1px solid var(--line); border-radius: 20px; text-decoration: none; color: var(--text); transition: all 0.2s; box-shadow: 0 8px 30px rgba(61,36,15,0.04); }}
  .links-banner a:hover {{ border-color: var(--accent); transform: translateY(-2px); box-shadow: 0 12px 40px rgba(61,36,15,0.08); }}
  .links-banner .link-icon {{ font-size: 24px; flex-shrink: 0; }}
  .links-banner .link-text {{ font-size: 14px; font-weight: 600; }}
  .links-banner .link-arrow {{ font-size: 16px; color: var(--accent); font-weight: 700; margin-left: auto; }}
  ...
  @media (max-width: 600px) {{ .container {{ padding: 36px 18px 60px; }} .summary-card, .news-card {{ padding: 20px 18px; }} .pico-grid {{ grid-template-columns: 1fr; }} footer {{ flex-direction: column; gap: 6px; text-align: center; }} .topic-name {{ width: 70px; font-size: 11px; }} .links-banner .link-row {{ flex-direction: column; }} }}
</style>
</head>
<body>
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>Tinnitus Brain &middot; 耳鳴研究文獻日報</h1>
  ...
  <div class="links-banner">
    ...
    <div class="link-row">
      <a href="https://buymeacoffee.com/CYlee" target="_blank">
        <span class="link-icon">☕</span>
  ...
  <footer>
    ...
    <span><a href="https://github.com/u8901006/tinnitus-brain">GitHub</a></span>
  </footer>
</div>
</body>
</html>"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate tinnitus daily report HTML"
    )
    ...
    if not papers_data or not papers_data.get("papers"):
        ...
        analysis = {
            ...
            "market_summary": "今日 PubMed 暫無新的耳鳴相關文獻更新。請明天再查看。",
            "top_picks": [],
            "all_papers": [],
            "keywords": [],
            "topic_distribution": {},
